...
CANONICAL_LANGS = ["rust", "go", "python", "bash"]
CANONICAL_TAGS = [
    "LLM",
    "CLIPS",
    "Solver",
...
def check_order(
    items: list[str], canonical: list[str], label: str, name: str
) -> list[str]:
    ...
    if indices != sorted(indices):
        expected = sorted(
            items, key=lambda x: canonical.index(x) if x in canonical else 999
        )
...
def main() -> int:
    ...
    for ex in data["examples"]:
        name = ex["name"]
        errors.extend(
            check_order(ex.get("languages", []), CANONICAL_LANGS, "languages", name)
        )
        errors.extend(
            check_order(ex.get("tech_tags", []), CANONICAL_TAGS, "tech_tags", name)
        )
